...
(function main() {
  ...
  if (summaryPath) {
    ...
    if (report.submission.reason) {
      mdLines.push(`- Note: ${report.submission.reason}`);
    }
    mdLines.push("");
    ...
    mdLines.push(
      `- Adjusted implementation score (flexible rules): **${report.implementationAdjusted} / ${report.implementationMax}**`
    );
    mdLines.push(
      `- Tasks attempted: **${report.attemptedTasks} / ${report.tasks.length}**`
    );
    ...
    mdLines.push("");
    mdLines.push("## Task-by-task breakdown");
    mdLines.push("");
    for (const t of report.tasks) {
